...
const options = {
  keystore: require("../../tls_certificates/server-keystore.p12"),
}
      ...
      const newClient = TcpSocket.connectTLS(
        {
          ...
          port,
          cert: true,
          ca: require("../../tls_certificates/server-cert.pem"),
        },
        () => {
          setIsConnected(true)
  ...
  const sendMessage = useCallback(
    (message: string | Buffer) => {
      if (client) {
        ...
        console.log("sent from the client: ", message)
      } else if (server) {
        serverSocket.write(JSON.stringify(message))
        console.log("Sent from the server: ", message)
      } else {
